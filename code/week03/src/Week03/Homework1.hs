...
mkValidator dat () ctx = traceIfFalse "conditions not met" (
                            (atOrBeforeDeadline && oneSigned)
                         || ((!atOrBeforeDeadline) && twoSigned))
    where
      info :: TxInfo
      info = scriptContextTxInfo ctx

      oneSigned :: Bool
      oneSigned = txSignedBy info $ unPaymentPubKeyHash $ beneficiary1 dat
